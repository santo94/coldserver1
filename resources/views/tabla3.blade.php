...
<thead class="gray">
    ...
    @php $fecha=date('Y-m-d'); @endphp
                <td data-order="{{$fecha}}"
    data-search="{{$fecha}}">{{$fecha}}</td>
                <td>{{ $ordenes->cliente->Nombre ?? 'Sin empresa' }}</td>
                
                <td>

                    @php
                    $fechaEvaluar = new DateTime($fecha); // la fecha que quieres evaluar
...
$(document).ready(function() {
  ...
  exportOptions: {
    ...
    $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
        ...
        var columnaOrden = data[4].toLowerCase();
